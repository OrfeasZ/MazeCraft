...
bool TextureManager::PrecachePNGTexture(const std::string& p_Name)
{
	uint32_t s_NameHash = Util::Utils::StringHash(p_Name);
	...
	Logger(Util::LogLevel::Info, "PreCaching PNG Texture '%s'.", p_Name.c_str());

	std::string s_FullPath = "/data/textures/" + p_Name + ".png";

	std::string s_TextureData;
	if (!VFS::FileSystem::GetInstance()->ReadFile(s_FullPath, s_TextureData))
	{
		Logger(Util::LogLevel::Warn, "Could not find PNG texture file '%s'.", p_Name.c_str());
		return false;
	}

	// Load texture using SOIL.
	GLuint s_TextureID = SOIL_load_OGL_texture_from_memory(
		(const unsigned char*) s_TextureData.data(),
		s_TextureData.size(),
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
		SOIL_FLAG_MIPMAPS | SOIL_FLAG_COMPRESS_TO_DXT | SOIL_FLAG_INVERT_Y | SOIL_FLAG_TEXTURE_REPEATS
	);

	if (s_TextureID == 0)
	{
		Logger(Util::LogLevel::Warn, "Could not load PNG texture file '%s'.", p_Name.c_str());
		return false;
	}
	...
	glGetTexLevelParameteriv(GL_TEXTURE_2D, 0, GL_TEXTURE_WIDTH, &s_Width);
	glGetTexLevelParameteriv(GL_TEXTURE_2D, 0, GL_TEXTURE_HEIGHT, &s_Height);

	auto s_RealTexture = new Rendering::Textures::Texture(s_TextureID, s_Width, s_Height, true);
	m_CachedTextures[s_NameHash] = s_RealTexture;
